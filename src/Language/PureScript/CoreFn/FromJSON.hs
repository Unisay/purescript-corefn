...
data ModuleWithVersion = ModuleWithVersion Version (Module Ann)
  deriving stock (Show)

instance FromJSON ModuleWithVersion where
  parseJSON = (uncurry ModuleWithVersion <$>) <$> moduleFromJSON

moduleWithoutVersion ∷ ModuleWithVersion → Module Ann
moduleWithoutVersion (ModuleWithVersion _version m) = m
...
annFromJSON _modulePath = withObject "Ann" annFromObj
 where
  annFromObj ∷ Object → Parser (Maybe Meta)
  annFromObj o = o .: "meta" >>= metaFromJSON
...
moduleFromJSON = withObject "Module" moduleFromObj
 where
  moduleFromObj o = do
    ...
    moduleBindings ← o .: "decls" >>= listParser (bindFromJSON modulePath)
