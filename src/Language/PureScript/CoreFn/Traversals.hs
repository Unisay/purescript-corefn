...
import Language.PureScript.CoreFn.Expr
  ...
  , CaseAlternative (..)
  , Expr (Abs, Accessor, App, Case, Let, Literal, ObjectUpdate)
...
everywhereOnValues f g h = (f', g', h')
 where
  ...
  g' (ObjectUpdate ann obj vs) =
    g (ObjectUpdate ann (g' obj) (map (fmap g') vs))
  g' (Abs ann name e) = g (Abs ann name (g' e))
  g' (App ann v1 v2) = g (App ann (g' v1) (g' v2))
  g' (Case ann vs alts) =
    g (Case ann (map g' vs) (map handleCaseAlternative alts))
...
traverseCoreFn f g h i = (f', g', h', i')
 where
  ...
  h' (ConstructorBinder a q1 q2 bs) =
    ConstructorBinder a q1 q2 <$> traverse h bs
  h' b = pure b
